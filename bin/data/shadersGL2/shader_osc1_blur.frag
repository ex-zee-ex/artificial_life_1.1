...
uniform float osc1_blur_amount;
uniform float osc1_blur_radius;

uniform float texmod_osc1_blur_amount;
uniform float texmod_osc1_blur_radius;
...
void main()
{
    ...
    float blur_mod=texmod_bright*texmod_osc1_blur_amount+osc1_blur_amount;
    
    //new version: blur amount is the weight of the neighborhood
    //blur radius will be the distance of the neighborhood
    
    float osc1_blur_radius_mod=texmod_osc1_blur_radius*texmod_bright+osc1_blur_radius;
    
    vec4 blur_kernel=texture2DRect(tex0,texCoordVarying+vec2(osc1_blur_radius_mod,osc1_blur_radius_mod));
    blur_kernel+=texture2DRect(tex0,texCoordVarying+vec2(0,osc1_blur_radius_mod));
    blur_kernel+=texture2DRect(tex0,texCoordVarying+vec2(-osc1_blur_radius_mod,osc1_blur_radius_mod));
    blur_kernel+=texture2DRect(tex0,texCoordVarying+vec2(-osc1_blur_radius_mod,0.0));
    blur_kernel+=texture2DRect(tex0,texCoordVarying+vec2(-osc1_blur_radius_mod,-osc1_blur_radius_mod));
    blur_kernel+=texture2DRect(tex0,texCoordVarying+vec2(0.0,-osc1_blur_radius_mod));
    blur_kernel+=texture2DRect(tex0,texCoordVarying+vec2(osc1_blur_radius_mod,-osc1_blur_radius_mod));
    blur_kernel+=texture2DRect(tex0,texCoordVarying+vec2(osc1_blur_radius_mod,0.0));
